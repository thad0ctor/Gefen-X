...
|---|---|---|---|---|---|---|
| Qwen3-0.6B | adamw_bf16 | 5e-5 | 1.438 | 5672 | 6.95 | 4.00 |
| Qwen3-0.6B | adamw8bit | 5e-5 | 1.477 | 5366 | 5.87 | 2.03 |
| Qwen3-0.6B | adamw4bit | 5e-5 | 1.465 | 5029 | 6.32 | 1.06 |
| Qwen3-0.6B | **gefen_fused** | 2e-5 | 1.489 | 5486 | 5.30 | 1.02 |
| Qwen3-0.6B | **gefen_muon** | 5e-5 | **1.427** | 3590 | **5.30** | **1.01** |
| Qwen3-1.7B | adamw_bf16 | 5e-5 | **1.218** | 3011 | 14.00 | 4.00 |
| Qwen3-1.7B | adamw8bit | 5e-5 | 1.244 | 2755 | 10.84 | 2.03 |
| Qwen3-1.7B | adamw4bit | 5e-5 | 1.241 | 2593 | 15.11 | 1.06 |
| Qwen3-1.7B | **gefen_fused** | 2e-5 | 1.281 | 2917 | 9.21 | 1.01 |
| Qwen3-1.7B | **gefen_muon** | 5e-5 | 1.239 | 1293 | **9.20** | **1.01** |

> Single-session sweep on RTX 3090 Ti (0.6B) / RTX 3090 (1.7B). `gefen_muon` uses the shipped `GefenMuonHybrid` defaults: `ns_schedule="tuned3"` (~1.4× the classic quintic, loss-neutral — see [the faster-NS lever](#experimental-lever-faster-newton-schulz-ns_schedule-fp8_ns); `ns_schedule="standard"` is the bit-identical quintic at ~0.7× this throughput) and `normuon=True` (throughput-free — see [the normuon lever](#quality-lever-per-neuron-2nd-moment-on-the-newton-schulz-output-normuon); `normuon=False` / harness `--no-muon-normuon` gives the pre-normuon config).
...
Gefen-fused converges stably and its eval curve tracks just above the AdamW cluster (a ~0.05–0.06 loss gap), with no instability at its fair LR. **Gefen-Muon closes that gap entirely**: its curve sits on the AdamW cluster — at 0.6B it edges past the best AdamW (−0.011) and at 1.7B it trails by only +0.021 (vs Gefen-fused's +0.051 / +0.063).
...
**Gefen-Muon** (`GefenMuonHybrid`, Muon on 2D hidden matrices + Gefen elsewhere, `match_rms_adamw` with a half-rate backup LR for the non-Muon params, `normuon` on) is the **best Gefen variant on loss** at both scales — 1.427 @0.6B (vs Gefen-fused 1.489; **beats** the best AdamW by 0.011) and 1.239 @1.7B (vs Gefen-fused's 1.281; +0.021 vs the best AdamW) — while tying Gefen-fused for the lowest peak VRAM and lowest optimizer state (~1.01 B/param). The cost is throughput: at the `tuned3` default Newton-Schulz still makes it the slowest optimizer (~0.65× Gefen-fused at 0.6B, ~0.44× at 1.7B). So **Gefen-fused is the speed+memory sweet spot; Gefen-Muon is the loss-recovery option** when you can afford the slowdown.
...
**Optional knobs.** `GefenMuonHybrid` defaults to the `tuned3` Newton-Schulz schedule (~1.4× step throughput, loss-neutral; pass `ns_schedule="standard"` for the classic quintic) and to `normuon=True` — a throughput-free per-neuron 2nd moment on the Newton-Schulz output that consistently nudges eval loss toward (or past) AdamW (see [the normuon lever](#quality-lever-per-neuron-2nd-moment-on-the-newton-schulz-output-normuon); `normuon=False` restores the pre-normuon trajectory). Two opt-in quality levers are off by default: `backup_2d_period_one=True` gives the embedding / LM head per-element 2nd moment to **match AdamW's loss at lower memory** (see [the experimental section](#experimental-lever-match-adamw-2nd-moment-on-embed--lm-head-backup_2d_period_one)), and `stochastic_round=True` switches the 8-bit momentum to unbiased stochastic rounding (free, loss-neutral).
...
> **On by default in `GefenMuonHybrid`** (the low-level `GefenMuon` primitive keeps `normuon=False`, mirroring the `ns_schedule` split). Throughput-free and ~free on memory; the published table above is measured with it, and `normuon=False` restores the pre-normuon trajectory.
...
Measured on the recommended config (fair-LR regime above, seed 0 + a seed-1 replication): at **0.6B** it beats plain recommended by **−0.007/−0.010** final eval on the two seeds — moving `GefenMuonHybrid` past the best AdamW — and at **1.7B** it trims the residual gap to AdamW by ~⅓ on the tail-mean of the last 10 evals (single-point finals at 1.7B are within the ±0.005–0.007 eval noise; the seed replication at 0.6B is the robust signal). A sign-agreement "cautious" mask over the NS output was benchmarked in the same sweep and clearly **lost** (+0.06 eval at 0.6B, −5% tok/s); it remains available as `cautious=True` for experimentation only.
