...
> **DeepSpeed ZeRO config.** Set `"zero_allow_untested_optimizer": true` and leave the config's `optimizer` section unset. With DeepSpeed's `zero_optimization.offload_optimizer`, also set `"zero_force_ds_cpu_optimizer": false` — otherwise raw DeepSpeed refuses to initialize, and accelerate-based launchers (axolotl) silently swap in DeepSpeed's own CPU Adam. Do not also call `Gefen.offload_state_()`: ZeRO owns the flattened parameter and offload lifecycle, so native Gefen offload fails closed if ZeRO replaces or clears its parameter groups.
...
Activation, restoration, movement, and checkpoint loading are fail-atomic: state is staged and validated before the live mapping is replaced. `move_state_(device=None)` is also available on `Gefen` and `GefenMuon`; without an explicit device it co-locates state with each live parameter. Native CPU offload supports plain `Gefen` on a single GPU, with `DistributedDataParallel`, and with FSDP2 `fully_shard` when every optimized parameter has one `Shard(0)` placement on a one-dimensional default-world CUDA DeviceMesh using NCCL. FSDP2 keeps ordinary local-shard state on CPU, stages it to each shard's CUDA device for the update, preserves the `rank_local_dtensor_v2` full-state DCP format, and synchronizes activation, movement/restoration, checkpoint, closure, and step-outcome failures across ranks; every rank must therefore enter each active-offload step and offload or checkpoint operation. Native `Gefen.offload_state_()` does not compose with DeepSpeed ZeRO's parameter ownership: leave it disabled and configure `zero_optimization.offload_optimizer` for CPU placement instead. Muon/hybrid optimizers, other DTensor meshes or placements, capturable/CUDA-graph execution, custom tensor-valued state extensions, multi-device-per-rank optimizers, and `torch.compile` execution remain rejected. Offload is synchronous; paged and overlapped transfer policies remain future work.
