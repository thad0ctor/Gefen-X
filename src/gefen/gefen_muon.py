...
class GefenMuon(Gefen):
    ...
    def step(self, closure=None):
        ...
        if self._gefen_codebook_process_group is not None:
            self._synchronize_codebook_scope_failure(
                capture_route_error, "capture routing preflight"
            )
        elif capture_route_error is not None:
            raise capture_route_error
        ...
        if (
            ...
            # This fixed-size header must precede local structural validation:
            # a malformed group on one rank is itself a coordinated rejection,
            # not permission for peers to enter a later optimizer collective.
            self._assert_sharded_mode_collective_intent_consistent()
