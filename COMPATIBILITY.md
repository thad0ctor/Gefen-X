...
**Fair-LR check (detectors).** A controlled 15-epoch, per-optimizer LR grid (3e-5 / 1e-4 / 3e-4) on the seven detectors AdamW led confirms the gap is mostly learning rate. Gefen's optimum sits at the lower 3e-5 (AdamW's at 3e-5–1e-4, consistent with the ≈0.6× fair LR), and at each optimizer's own best LR the shortfall shrinks to ≤0.01 mAP@50-95 on five of seven — with Gefen ahead on RetinaNet (+0.010) and RT-DETR (+0.004) — and ≤0.03 on the other two (YOLOS −0.030, Deformable-DETR −0.030). Deformable-DETR is the clearest case: a −0.211 gap at the shared 1e-4 (Gefen overshoots) collapses to −0.030 once each optimizer uses its own LR. The main table above uses one matched LR per row; a per-optimizer LR would narrow or erase most of those detector gaps.
