...
The native format is deliberately same-topology only and currently requires one 1-D DeviceMesh spanning the default process-group world. Plain Gefen additionally exposes `GefenDCPState`, a standalone `torch.distributed.checkpoint` Stateful wrapper for one-dimensional default-world `Shard(0)` DTensors. It writes dense momentum and second-moment DTensors without gathering the complete optimizer state onto one rank; DCP can therefore reshard a checkpoint from N ranks to M ranks. Load validates every field before mutation and projects the restored state into an exact period-one Gefen representation. Muon, Hybrid, multidimensional meshes, subgroups, non-`Shard(0)` placements, and the native full-state format remain same-topology and fail closed.
