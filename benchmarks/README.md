...
> **Note on the published `gefen_muon` loss numbers:** they predate `normuon`, so reproducing them bit-for-bit needs `--no-muon-normuon` (results are then labeled `recommended-no-normuon`). Default runs include normuon and should come out slightly better (0.6B: −0.007/−0.010 final eval over two seeds; 1.7B: ~⅓ of the residual gap to AdamW on the tail-mean of the last 10 evals).
