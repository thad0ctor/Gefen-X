...
`run.sh` applies the **recommended Gefen-Muon config by default** — `adjust_lr_fn=match_rms_adamw`, `backup_lr = 0.5 × the cell LR`, and `backup_1d_period_one`. Override via config keys (`muon_adjust`, `muon_backup_lr_fraction`, `muon_backup_1d`) or flags (`--muon-adjust`, `--muon-backup-lr-frac`, `--no-muon-backup-1d`). The newer throughput-free `normuon` loss lever (per-neuron 2nd moment on the Newton-Schulz output; beats plain recommended at 0.6B and trims the 1.7B gap — see the `GefenMuonHybrid` docstring) is opt-in via `--muon-normuon` / `muon_normuon: true` and is labeled `recommended+normuon` in the results; the published table predates it. See [the optimizer-sweep README](optimizer-sweep/README.md) for the full flag list.
