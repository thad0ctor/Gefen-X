...
`run.sh` applies the **recommended Gefen-Muon config by default** — `adjust_lr_fn=match_rms_adamw`, `backup_lr = 0.5 × the cell LR`, `backup_1d_period_one`, and (via the `GefenMuonHybrid` default) `normuon`, the throughput-free per-neuron 2nd moment on the Newton-Schulz output (beats plain recommended at 0.6B and trims the 1.7B gap — see the `GefenMuonHybrid` docstring). Override via config keys (`muon_adjust`, `muon_backup_lr_fraction`, `muon_backup_1d`, `muon_normuon`) or flags (`--muon-adjust`, `--muon-backup-lr-frac`, `--no-muon-backup-1d`, `--no-muon-normuon`). See [the optimizer-sweep README](optimizer-sweep/README.md) for the full flag list.

> **Note on the published `gefen_muon` numbers:** they are measured with the shipped defaults, including `normuon`. Pass `--no-muon-normuon` for the pre-normuon config (results are then labeled `recommended-no-normuon`); the isolated lever A/B (0.6B: −0.007/−0.010 final eval over two seeds; 1.7B: ~⅓ of the residual gap to AdamW on the tail-mean of the last 10 evals) is documented in the main README's normuon section.
