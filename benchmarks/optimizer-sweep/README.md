...
- **`gefen_muon` follows the shipped `GefenMuonHybrid` defaults** unless
  ...
  output, on by default). Published results that predate normuon reproduce with
  `--no-muon-normuon`; the effective value is recorded in each result line
  (`muon_flags.normuon` / `normuon_explicit`).
