...
- Add fail-atomic `move_state_()` for `Gefen` and `GefenMuon`, plus synchronous CPU-authoritative optimizer-state offload for plain `Gefen` through `offload_state_()` and `restore_state_()`. Native offload is bit-exact for single-GPU, DDP, and FSDP2 `fully_shard` training when every optimized parameter has one `Shard(0)` placement on a one-dimensional default-world CUDA DeviceMesh using NCCL; FSDP2 stores ordinary local-shard state on CPU, preserves active offload and the `rank_local_dtensor_v2` format across full-state DCP resume, and synchronizes activation, movement/restoration, checkpoint, closure, and step-outcome failures across ranks. DeepSpeed ZeRO parameter-group takeover fails closed with guidance to use `zero_optimization.offload_optimizer`, while ordinary plain-Gefen ZeRO training remains supported with native offload disabled. Active offload validates its state on every step and poisons every participating FSDP2 rank after any post-preflight step failure; a committed known-good load clears poison and returns a CPU-mapped codebook to the parameter device.
