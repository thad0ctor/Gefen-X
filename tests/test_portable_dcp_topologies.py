...
def _plain_flat_save_worker(
    ...
    try:
        ...
        members = tuple("save:{}".format(index) for index in range(world_size))
        group = ProcessGroupIdentity("plain_flat_save", members)
        identity = ParameterIdentity(_PLAIN_FQN, (2, 4))
        ...
        local = torch.nn.Parameter(
            _plain_initial_parameter().reshape(-1)[start:stop].clone()
        )
        optimizer = _plain_optimizer(original, deterministic=True)
        ...
        _seed_plain_state(
            optimizer,
            local,
            _plain_momentum().reshape(-1)[start:stop].clone(),
            _plain_second_moment().reshape(-1)[start:stop].clone(),
        )
        save_portable_dcp(
            optimizer,
            checkpoint_process_group=checkpoint_binding,
            storage_writer=FileSystemWriter(checkpoint_dir),
            transaction_id="plain-flat-cross-world-save",
            limits=_limits(),
        )
        result_queue.put(
            {
...
def test_portable_dcp_two_rank_flattened_checkpoint_reshards_to_four_rank_world(tmp_path):
    checkpoint_dir = str(tmp_path / "plain-flat-two-to-four")
    source_lengths = (3, 5)
    save_results = _run_phase(
        _plain_flat_save_worker,
        2,
        checkpoint_dir,
        source_lengths,
    )
    assert all("fatal_error" not in result for result in save_results), save_results
    assert [result["length"] for result in save_results] == list(source_lengths)
    assert all(result["saved"] for result in save_results)

    target_lengths = (1, 2, 2, 3)
    load_results = _run_phase(
        _plain_flat_load_worker,
        4,
        checkpoint_dir,
    ...
    assert all("fatal_error" not in result for result in load_results), load_results
    assert [result["length"] for result in load_results] == list(target_lengths)
    assert all(result["restored_exact"] and result["next_step_exact"] for result in load_results), load_results
